...
<!-- - Population considered (`subpop`). It is either "elders" or "infants". When it is `NA`, it probably means that the whole population is considered. However, it might also be the case that the population considered was just not detected. There are `r articles_low_adequate |> unnest(subpop) |> count(subpop) |> filter(is.na(subpop)) |> pull(n)` missing observations. We thus want to compare these specific population to the rest of the population. -->
<!-- - Outcomes (`outcome`). It is either "emergency" or "mortality" (or missing or non-detected. There are `r articles_low_adequate |> unnest(outcome) |> count(outcome) |> filter(is.na(outcome)) |> pull(n)`). We thus want to compare these outcomes together, *ie* excluding missing values -->
<!-- - Fields (`subject_area`).  -->
...
modelsummary(reg_power, gof_omit = "IC|Adj|Log", stars = TRUE, coef_rename = \(x) str_remove(x, "TRUE")) 
```
...
- Studies interested considering air quality indexes seem to have higher power on average (and studies considering black carbon may have lower power). However, only `r data_reg |> count(doi, air_quality_index) |> filter(air_quality_index) |> nrow()` studies in our dataset consider such a pollutant.
...
modelsummary(reg_adeq, gof_omit = "IC|Adj|Log", stars = TRUE, coef_rename = \(x) str_remove(x, "TRUE"))
```
