...
description: |
  Under construction - An Imaginary but Illustrative Experiment.
author:
...
loading custom ggplot theme
coder <- "vincent"
if (coder == "leo") {
  blue <- "#219ebc"
  orange <-  "#fe6847"
  source(here::here("2.scripts", "script_theme_tufte.R"))
  theme_custom <- theme_tufte
} else {
  source(here::here("2.scripts", "mediocretheme_function.R"))
  theme_custom <- theme_mediocre
  blue <- "#00313C"
  orange <- "#FB9637"
}
```
...
```{r display_science_table, include=FALSE}
...
```{r plot_density_pot_out, fig.width=10, fig.height=5, include=FALSE}
...
data_scientist_treatment_assignment <-
  readRDS(here::here("1.data", "data_scientist_treatment_assignment.RDS"))
...
saveRDS(results_10000_experiments, here::here("1.data", "data_results_10000_experiments.RDS"))
```  
...
data_results_10000_experiments <-
  readRDS(here::here("1.data", "data_results_10000_experiments.RDS"))
...
graph_experiment_results_dots <-
  ...
  geom_point(shape = 16,
             size = 1,
             alpha = 0.6) +
  scale_color_manual(name = "Statistically Significant at the 5% Level", values = c(blue, orange)) +
...
```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.width=10, fig.height=5, dev = "CairoPNG"}
# create the graph
graph_ci <- data_results_10000_experiments %>%
  ...
  scale_colour_manual(name = "Statistically Significant at the 5% Level", values = c(blue, orange)) +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 10)) +
...
# save simulation results
saveRDS(results_large_sample_experiments, here::here("1.data", "data_results_large_sample_experiments.RDS"))
```  
...
data_results_large_sample_experiments <-
  readRDS(here::here("1.data", "data_results_large_sample_experiments.RDS"))
```  
...
With a sample size three times bigger, the statistical power of the experiment has increased but is still very low. Consequently, the exaggeration factor has decreased from `r type_m_large_exp` to `r `r type_m_large_exp`. The probability to make a type S error has been reduced a lot and is close to 1. The coverage rate of confidence intervals has increased but is still far from its 95% theoretical target.
