...
```{r}
draw_treated_days <- function(dates, p_treat = 0.5, treatment_type = "random_RCT") {
  
  if (treatment_type == "random_RCT") {
    treated <- rbernoulli(length(dates), p_treat)
  } else if (treatment_type == "random_ITS") {
    begin_treatment <- dates %>% 
      unique() %>% 
      sample(1)
    treated <- (dates >= begin_treatment)
  } 
