...
compute_simulations <- function(data, n_days_study = 1200, p_treat = 0.5, treatment_type = "random", percent_effect_size = 0.5, formula, n_iter = 1000) {
  ...
  all_est %>%
    ...
    select(formula:n_iter, estimate, p_value)
} 
````
...
summarise_simulations <- function(data) {
  ...
  data %>% 
    group_by_("formula", "treatment_type", "n_days_study", "p_treat", "effect_size", "n_iter") %>%
    mutate(power = mean(p_value <= 0.05)*100) %>% 
    ungroup() %>%
    # filter(p_value <= 0.05) %>%
    group_by_("formula", "treatment_type", "n_days_study", "p_treat", "effect_size", "n_iter") %>%
    summarise(
      power = unique(power),
      type_m = mean(abs(estimate)/effect_size),
...
# t <- total_data %>%
#   compute_simulations(
#     formula = "deaths_all_causes ~ treated + rainfall_height",
#     n_iter = 50
#   )

summarise_simulations(t)
```
