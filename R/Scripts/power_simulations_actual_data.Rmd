...
sim_param <- prepare_sim_param(sim_param_unique, n_iter = 1)
...
```{r eval=FALSE}
sim_decomp_nobs <- NULL
save_every <- 10000
future::plan(multicore)

tic()
for (i in 1:ceiling(nrow(sim_param_decomp_nobs)/save_every)) {
  params_slice <- sim_param_decomp_nobs %>% 
    slice((1 + save_every*(i-1)):(save_every*i))
  
  with_progress({
    p <- progressor(steps = nrow(params_slice))
    
    sim_decomp_nobs_to_save <- future_pmap_dfr(
      params_slice,
      compute_simulation,
      data = nmmaps_data,
      progressbar = p,
  ...
  print(paste("Iteration =", i*save_every))
  sim_decomp_nobs <- sim_decomp_nobs %>% rbind(sim_decomp_nobs_to_save)
  saveRDS(
    sim_decomp_nobs_to_save, 
    here("R", "Outputs", "Sim_save", str_c("sim_decomp_nobs_saved_", i,".RDS"))
  )
}
toc()
...
```{r}
sim_decomp_ptreat <- NULL
save_every <- 10000
future::plan(multicore)

tic()
for (i in 1:ceiling(nrow(sim_param_decomp_ptreat)/save_every)) {
  params_slice <- sim_param_decomp_ptreat %>% 
    slice((1 + save_every*(i-1)):(save_every*i))
  
  with_progress({
    p <- progressor(steps = nrow(params_slice))
    
    sim_decomp_ptreat_to_save <- future_pmap_dfr(
      params_slice,
      compute_simulation,
      data = nmmaps_data,
      progressbar = p,
  ...
  print(paste("Iteration =", i*save_every))
  sim_decomp_ptreat <- sim_decomp_ptreat %>% rbind(sim_decomp_ptreat_to_save)
  saveRDS(
    sim_decomp_ptreat_to_save, 
    here("R", "Outputs", "Sim_save", str_c("sim_decomp_ptreat_saved_", i,".RDS"))
  )
}
toc()
