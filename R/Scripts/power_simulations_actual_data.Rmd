...
```{r}
select_study_period <- function(data, n_days = 2000, n_cities = 68) {
  dates <- data[["date"]]
...
draw_treated <- function(data, p_obs_treat = 0.5, quasi_exp = "random_days") {
  ...
  if (quasi_exp == "random_days") {
    ...
    data[["treated"]] <- (num_date >= quantile(num_date, 1 - p_obs_treat))
  } else if (quasi_exp == "local") {
    treated_cities <- unique(data[["city"]]) %>%
      sample(size = round(length(.) * min(p_obs_treat * 2, 1)))
    ...
    data <- data %>%
      ...
      mutate(
        treated = (.data$date > sample(
          seq.Date(
            min(.data$date) + (1 - 2*p_obs_treat)*length(.data$date),
            max(.data$date), "day"
          ), 1))
      ) %>%
      ungroup() 
    ...
    bw <- floor(min(p_obs_treat, 0.5) * length(dates))
...
create_y1 <- function(data,
  ...
  if (id_method %in% c("DID", "ITS", "RDIT", "RDD")) {
...
compute_true_effect <- function(data, id_method, percent_effect_size, dep_var) {
  ...
  if (id_method == "DID") {#ATET
    ...
    true_effect <- data %>% 
      ...
      summarise(mean(y1 - y0, na.rm = TRUE)) %>% 
      as.numeric()
  } else if (str_starts(id_method, "IV|OLS")) {
...
nmmaps_data %>%
  compute_simulation(
    data = .,
    formula = "death_total ~ treated + post + city_treated + temperature + temperature_squared |  month^year + weekday",
    quasi_exp = "local",
    id_method = "DID",
    n_days = 4000,
    n_cities = 68,
...
vect_n_days <- c(100, 500, 1000, 2000, 3000)
vect_n_cities <- c(1, 4, 20, 68)
vect_p_obs_treat <- c(0.5, 0.2, 0.01)
vect_percent_effect_size <- c(0.1, 0.5, 1, 10)
vect_formula <- c(
  "death_total ~ treated", 
...
vect_of_vect_param <- c("vect_n_days", "vect_n_cities", "vect_p_obs_treat", "vect_percent_effect_size", "vect_formula")
...
sim_param_unique_clean <- sim_param_base %>% #sim_param_unique %>%
  crossing(vect_quasi_exp) %>%
  rename(quasi_exp = vect_quasi_exp) %>%
  # mutate(p_obs_treat = ifelse(str_starts(quasi_exp "alert"), p_obs_treat/10, p_obs_treat) %>% 
  mutate(
    id_method = case_when(
  ...
  #adapting parameters
  filter(!(n_cities == 1 & quasi_exp == "local")) %>% 
  filter(!(quasi_exp %in% c("local", "national") & p_obs_treat < 0.1)) %>% 
  mutate(
    p_obs_treat = case_when(
      id_method == "RDD" ~  p_obs_treat/5,
      
      id_method == "DID" ~  p_obs_treat/2,
      TRUE ~ p_obs_treat
    )
  ) %>% 
  # filter(!str_starts(id_method, "OLS|IV")) %>%
  filter(str_starts(id_method, "RCT"))
  # filter(id_method == "RCT" & str_starts(formula, "death"))

sim_param <- sim_param_unique_clean %>% 
...
# saveRDS(all_simulations, "R/Outputs/all_simulations.RDS")
```
...
summarise_simulations <- function(data) {
  ...
  data %>% 
    group_by(formula, quasi_exp, n_days, n_cities, p_obs_treat, percent_effect_size, id_method) %>%
    summarise(
...
all_simulations %>% 
  ...
  ggplot() +
  geom_histogram(aes(x = estimate, fill = significant)) + 
  geom_vline(xintercept = mean(all_simulations$true_effect))
...
group_by(.data$city) %>% 
      mutate(
        treatment_effect = rpois(
            n(),
            mean(.data[[dep_var]], na.rm = TRUE) * percent_effect_size / 100
          ) %>% suppressWarnings() #warnings when is.na(dep_var) eg rpois(1, NA)
        y1 = ifelse(id_method == "RCT", .data[[dep_var]] + treatment_effect, .data[[dep_var]] - treatment_effect)
      ) %>% 
      ungroup() %>% 
