...
```{r eval=FALSE}
compute_all_simulations <- function(data) {
...
with_progress({
  simulations <- compute_all_simulations(sim_param %>% select(-sim_id) %>% slice(1:10))
})

# tic()
# simulations <- future_pmap_dfr(
#   sim_param %>% select(-sim_id) %>% filter(id_method == "IV_0.4"), 
#   compute_simulation, 
#   data = nmmaps_data,
#   .id = "sim_id",
#   .options = furrr_options(seed = TRUE),
