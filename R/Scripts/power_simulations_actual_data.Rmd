...
```{r}
draw_treated_days <- function(data, p_treat = 0.5, treatment_type = "RCT", pollutant = "pm10") {
...
```{r}
create_y1 <- function(dep_var, percent_effect_size = 0.5, treatment_type = "RCT") {
...
```{r}
compute_one_simulation <- function(data, n_days_study = 200, p_treat = 0.5, treatment_type = "RCT", percent_effect_size = 0.5, formula = "deaths_all_causes ~ treated") {
...
compute_one_simulation(total_data %>% filter(!is.na(deaths_all_causes)), formula = "deaths_all_causes ~ treated")

compute_simulations <- function(data, n_days_study = 200, p_treat = 0.5, treatment_type = "RCT", percent_effect_size = 0.5, formula = "deaths_all_causes ~ treated", n_iter = 1000) {
...
We can then run the simulations, using the function `compute_simulations`. As mentioned, we want to test different values for the parameters of this functions. For instance, we want to run simulations for different effect sizes. We thus create a table displaying in each row a set of parameters we want to have a simulation for, `sim_param`.

To build `sim_param`, we first define the different values we want to consider for each parameter. Finally, we cross that with `vect_treatment_type` to get all set of parameter for each type of treatment.

```{r}
vect_n_days_study <- c(1000, 100, 500)
vect_p_treat <- c(0.5, 0.01, 0.1)
vect_treatment_type <- c("RCT", "RDD", "DID")
...
```{r eval=FALSE}
all_simulations <- pmap_dfr(sim_param, compute_simulations, data = total_data)
