...
draw_treated_days <- function(data, p_treat = 0.5, treatment_type = "RCT", pollutant = NULL) {
  ...
  if (treatment_type == "RCT") {
    ...
    begin_treatment <- data[["date"]] %>% 
      unique() %>% 
      sample(1)
    treated <- (data[["date"]] >= begin_treatment)
    ...
    treated <- (data[[pollutant]] >= quantile(data[[pollutant]], p_treat))
  }
...
```{r}
create_y_1 <- function(dep_var, percent_effect_size = 0.5) {
...
summarise_simulations <- function(data) {
  ...
  data %>% 
    group_by(formula, treatment_type, n_days_study, p_treat, percent_effect_size, n_iter, true_effect) %>%
    summarise(
      power = mean(p_value <= 0.05)*100, 
      bias = mean(estimate) - true_effect,
      average_p_value = mean(p_value),
      average_n_obs = mean(n_obs, na.rm = TRUE),
      type_m = mean(ifelse(p_value < 0.05, estimate, NA), na.rm = TRUE)/true_effect,
      type_s = (sum(sign(estimate) != sign(true_effect))/n())*100,
      .groups	= "drop"
    ) %>% 
    ungroup()
} 
