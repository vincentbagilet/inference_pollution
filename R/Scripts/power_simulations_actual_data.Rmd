...
sim_param_unique_clean <- sim_param_unique %>%
  ...
  # filter(str_starts(id_method, "DID")) %>% 
  # filter(n_days == 3000) %>% 
  # mutate(percent_effect_size = 50)
...
for (i in 1:ceiling(nrow(sim_param)/save_every)) {
  ...
  with_progress({
    ...
    simulations_output_to_save <- future_pmap_dfr(
      params_slice,
      compute_simulation,
      data = nmmaps_data,
      progressbar = p,
  ...
  simulations_output <- simulations_output %>% rbind(simulations_output_to_save)
  saveRDS(
    simulations_output_to_save, 
    here("R", "Outputs", "Sim_save", str_c("sim_output_saved_", i,".RDS"))
  )
}
...
```{r eval=FALSE}
summary_simulations <- summarise_simulations(simulations_output)
