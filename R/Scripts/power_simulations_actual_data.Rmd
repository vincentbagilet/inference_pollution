...
create_y1 <- function(data,
  ...
  if (id_method %in% c("RCT", "DID", "ITS", "RDIT", "RDD")) {
    ...
    reg$coefficients[[pollutant]] <- log(1 + percent_effect_size/100) #to get beta as the increase in percent
    res <- reg$residuals
...
# create_y1(draw_treated(nmmaps_data), id_method = "IV_0.05", formula = "resp_total ~  temperature | month + year + weekday + city | co ~ treated") %>% select(resp_total, y1)
```
...
estimate_model(draw_treated(nmmaps_data), "resp_total ~ co + temperature | city ")
...
compute_simulation <- function(data,
  ...
  updated_fml <- ifelse(
    ...
    str_replace(iv, dep_var, "y1"),
    str_replace(iv, dep_var, "yobs")
  )
...
summarise_simulations <- function(data) {
  ...
  data %>% 
    group_by(formula, quasi_exp, n_days, n_cities, p_treat, percent_effect_size, id_method) %>%
    summarise(
