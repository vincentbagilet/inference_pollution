...
Then, we create a function to draw the treatment. This function adds a boolean vector to our data, stating whether each observation is in the treatment group or not. We consider three quasi-experiments: 
- Random days, corresponding to a random allocation of the treatment
- Air pollution alerts
- No quasi-experiment

For each treatment, we define the proportion of treated observations (`p_obs_treat`). 

Note that for the alert, we consider a RDD and therefore will restrict our sample. 

For the air pollution alert, we draw a threshold position from a uniform distribution from 0.2 to 0.4 in order to have enough observations in our data set. If we pick the threshold position too high up in the distribution of CO concentration, we may end up with observations very far away from the samples. For each city, we find the corresponding threshold and define a bandwidth such that `p_obs_treat` observations of the total sample are treated. Observations outside the bandwidth get a `NA` for `treated`. Thus, after calling this function, we need to filter out observations with a `NA` for `treated`.
...
We then create the fake output, `y_fake`. The generative process depends on the identification method. We use different methods to generate fake mortality data: 
...
- For the IV, we use the same method as for the OLS but previously modified the value of pollutant concentration to correspond to be affected by the treatment. The intensity is set equal to the parameter `iv_intensity` and we add some white noise drawn from a normal distribution (0, 0.1).
...
We need to compute the true effect for each simulation. The different identification methods aims to identify different estimands (ATE or ATET). We therefore write a short function to compute this true effect.
...
compute_true_effect <- function(data, id_method, percent_effect_size, dep_var) {
  ...
  if (id_method %in% c("OLS", "IV")) {
    ...
    true_effect <- mean(data$y_fake - data$y0, na.rm = TRUE)
  }
...
compute_simulation <- function(data,
  ...
  sim_data <- data %>%
    ...
    create_fake_output(percent_effect_size, id_method, iv_intensity, formula) %>% 
    mutate(
      yobs = y_fake * treated + y0 * (1 - treated)
    ) %>%
    filter(!is.na(treated)) #not necessary bc dropped in lm()
  
  updated_fml <- ifelse(
    id_method %in% c("OLS", "IV"),
    str_replace(formula, dep_var, "y_fake"),
...
nmmaps_data %>%
  compute_simulation(
    data = .,
    formula = "death_total ~ co + temperature + temperature_squared | city + month^year + weekday",
    quasi_exp = "none",
    id_method = "OLS",
    iv_intensity = 0.5,
    n_days = 1000,
...
vect_of_vect_param <- c("vect_n_days", "vect_p_obs_treat", "vect_percent_effect_size", "vect_iv_intensity", "vect_formula")
...
We want to compute our simulations for this set of parameters for every quasi experiment. Each quasi experiment has an associated identification strategy. Note that, in order to identify the effect of interest, we need to consider different types of equations, depending on the identification method considered. Then, for each set of parameters we want to run many iterations of the simulation, `n_iter`. It will enable us to loop `compute_simulation` directly on `sim_param_evol`.
...
We can then run the simulations for each set of parameter, using a `pmap_dfr` function. We wrote a function to do that to save intermediary outcomes.
...
sim_evol <- run_all_sim(nmmaps_data, sim_param_evol, save_every = 10000, "sim_evol")

# beepr::beep(1)

# saveRDS(sim_evol, here("R", "Outputs", "sim_evol.RDS"))
```
...
We then replicate this analysis for a smaller and more realistic number of observations (10 cities and 1000 days). 
