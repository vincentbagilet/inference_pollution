...
```{r}
draw_treated_days <- function(data, p_treat = 0.5, treatment_type = "RCT", pollutant = "pm10", n_days_study = 200) {
  ...
  if (treatment_type == "RCT") {
    ...
    treated <- data %>%
      group_by(city) %>%
      mutate(
        threshold = as.numeric(quantile(.data[[pollutant]], p_treat)),
        ...
        treated =
          ifelse(
            threshold >= as.numeric(str_extract_all(in_bw, "([:digit:]|\\.|-)+(?=,)")) &
            threshold < as.numeric(str_extract_all(in_bw, "(?<=,)([:digit:]|\\.)+")),
          treated, NA)
      ) %>%
      ungroup() %>%
...
total_data %>%
      group_by(city) %>%
      mutate(
        threshold = as.numeric(quantile(pm10, 0.5)),
        treated = (pm10 >= threshold),
        in_bw = cut_width(
          pm10,
          width = 200,
          center = unique(threshold)
        ),
        above_th = (threshold >= as.numeric(str_extract(in_bw, "([:digit:]|\\.|-).+(?=,)"))), 
        below_th = (threshold < as.numeric(str_extract(in_bw, "(?<=,)([:digit:]|\\.)+"))),
        treated = ifelse(above_th & below_th, treated, NA)
      ) %>%
      ungroup() %>%
      .$treated 
```
