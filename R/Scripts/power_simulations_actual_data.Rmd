...
# nmmaps_data %>% 
#   select_study_period() %>%
#   create_y1(id_method = "OLS", formula = "death_total ~ co + temperature | city + month^year + weekday") %>% 
#   feols(data = ., fml = y1 ~ co + temperature | city + month^year + weekday) 
...
sim_param_unique_clean <- sim_param_base %>% #sim_param_unique %>%
  ...
  filter(str_starts(id_method, "RDD"))
