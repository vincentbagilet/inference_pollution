...
us_data <- readRDS(here::here("R", "Inputs", "nmmaps_data_simulations.rds"))
...
us_data <- us_data %>% 
  ...
  mutate(
    city = as.factor(str_c(city_name, abbreviation, sep = ", "))
  ) %>% 
  select(-abbreviation, -city_name) %>% 
  select(city, date, everything()) 
```
...
```{r}
draw_study_period <- function(data, n_days_study = 200, n_cities = 66) {
  dates <- data[["date"]]
  err_proof_n_days_study <- min(n_days_study, unique(dates))
  cities <- data[["city"]]
  ...
  begin_study <- sample(
    seq.Date(
      min(dates), 
      max(dates) - err_proof_n_days_study, 
      "day"
    ), 1)
  ...
  data[["study_period"]] <- (dates_kept & cities_kept)
  return(data)
}
```
...
test_treatment_data <- us_data %>% 
  draw_study_period(n_days_study = 500, n_cities = 6) %>% 
  filter(study_period) %>%
  select(-study_period) %>%
...
create_y1 <- function(data,
  ...
  } else if (str_starts(quasi_exp, "OLS")) {
    pollutant <- str_extract(quasi_exp, "(?<=_).+")
    fml <- Formula::as.Formula(formula)
    actual_fml <- formula(fml, rhs = -3)
    
    reg <- feols(data = data, fml = actual_fml) 
...
# create_y1(total_data, quasi_exp = "OLS_co", formula = "log(resp_total) ~ co + temperature | city + day_of_week + month^year") %>% select(deaths_all_causes, y1)
```
...
# estimate_model(total_data, "resp_total ~ temperature")
```
...
sim_param_base <- tibble(
  ...
  p_treat = 0.2,
  percent_effect_size = 0.5, 
  formula = "resp_total ~ treated + temperature + I(temperature^2) | city + month + year + day_of_week"
)
...
vect_formula <- c(
  ...
  "resp_total ~ treated | city + month + year + day_of_week",
  "resp_total ~ treated + temperature + I(temperature^2) | city + month + year + day_of_week"
)
```
...
```{r}
vect_quasi_exp <- c("random_days", "national", "local")#, "national_short", "rolling") #, "alert_co"
n_iter <- 100
...
<!-- vect_n_days_study <- seq(500, 1000, 100) -->
<!-- vect_percent_effect_size <- seq(0.5, 1.5, 0.1) -->

<!-- sim_param_heatmap <- sim_param_base %>%  -->
<!--   select(-n_days_study, -percent_effect_size) %>%  -->
<!--   crossing(vect_n_days_study, vect_percent_effect_size) %>% -->
...
<!--   filter(percent_effect_size > 0.4 & n_days_study > 400) %>% -->
<!--   ggplot(aes(x = n_days_study, y = percent_effect_size)) +  -->
