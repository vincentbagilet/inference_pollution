...
Binary treatment (thermal inversion, high/low wind speed etc) 
...
find_pollutant <- function(formula) {
    ...
    pollutant <- pollutants_list[pollutants_list %in% all.vars(formula[[3]])]
    return(pollutant)
}
...
create_y1 <- function(data,
  ...
  if (id_method %in% c("RCT", "DID", "ITS", "RDIT", "RDD")) {
    dep_var <- all.vars(fml[[2]])
    
    data[["y1"]] <- data[[dep_var]] + 
      rpois(
        length(data[[dep_var]]), 
    ...
    pollutant <- find_pollutant(actual_fml)
    
    reg <- feols(data = data, fml = actual_fml, combine.quick=FALSE) 
...
# create_y1(us_data, id_method = "OLS", formula = "log(resp_total) ~ co + temperature | city") %>% select(resp_total, y1)
```
...
estimate_model <- function(data, formula) {
  ...
  param_of_interest <- ifelse(
    "treated" %in% all.vars(actual_fml[[3]]), 
    "treatedTRUE", 
    find_pollutant(actual_fml)
  )
...
# estimate_model(us_data, "resp_total ~ co + temperature | city")
```
...
compute_simulation <- function(data,
  ...
  if (id_method == "DID") {
    ...
    true_effect <- percent_effect_size
  } else { 
...
test <- us_data %>%
  compute_simulation(
    formula = "log(resp_total) ~ co + temperature | city",
    quasi_exp = "none",
    id_method = "OLS",
    n_days = 500
    # quasi_exp = "local"
...
vect_of_vect_param <- c("vect_n_days", "vect_p_treat", "vect_percent_effect_size", "vect_formula")
