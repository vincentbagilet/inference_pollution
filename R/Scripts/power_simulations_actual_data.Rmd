...
sim_param_decoupling <- tibble(n_cities = c(1, 5, 15, 34)) %>% 
  crossing(n_obs = c(1000, 2000, 4000)) %>% 
  mutate(n_days = floor(n_obs/n_cities)) %>% 
  select(-n_obs) %>% 
...
with_progress({
  ...
  simulations_decomp_nobs <- future_pmap_dfr(
    sim_param,
    compute_simulation,
    data = nmmaps_data,
    progressbar = p,
...
# saveRDS(simulations_decomp_nobs, here("R", "Outputs", "simulations_decomp_nobs.RDS"))

summary_decomp_nobs <- summarise_simulations(simulations_decomp_nobs)
