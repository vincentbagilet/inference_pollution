...
```{r eval=FALSE}
compute_all_simulations <- function(params) {
...
with_progress({
  simulations <- compute_all_simulations(sim_param %>% select(-sim_id))
})
...
# saveRDS(all_simulations, "R/Outputs/data_simulations.RDS")
```
...
summarise_simulations <- function(data) {
  ...
  data %>% 
    group_by(formula, quasi_exp, n_days, n_cities, n_obs, p_treat, percent_effect_size, id_method) %>%
    summarise(
