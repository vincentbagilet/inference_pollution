...
# create_y1(draw_treated(us_data), id_method = "IV_0.05", formula = "log(resp_total) ~  temperature | city | co ~ treated") %>% select(resp_total, y1)
```
...
estimate_model <- function(data, formula) {
  ...
  param_of_interest <- 
    ifelse(str_count(pasted_formula, "\\|") == 2, str_c("fit", find_pollutant(fml), sep = "_"), 
           ifelse("treated" %in% all.vars(fml), "treatedTRUE", find_pollutant(fml)))
  
  print(param_of_interest)
...
test <- us_data %>% 
  draw_treated() %>% 
  compute_simulation(
    formula = "resp_total ~ co + temperature | city",
    quasi_exp = "random_days",
    id_method = "OLS",
    n_days = 500
    # quasi_exp = "local"
