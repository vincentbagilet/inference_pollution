...
draw_treated_days <- function(data, p_treat = 0.5, treatment_type = "RDD_pm10") {
  ...
  if (treatment_type == "RCT") {
    ...
    treated <- (data[["city"]] %in% treated_cities & data[["date"]] >= median(data[["date"]]))
...
test_treatment_data %>% 
  ggplot() +
  geom_point(aes(x = date, y = pm10, color = treated_RCT)) +
  facet_wrap(~city) +
  scale_color_discrete(na.value = "grey") +
  labs(
    title = "Treatment assignation for RCT", 
    x = "Date", 
...
test_treatment_data %>% 
  ggplot() +
  geom_point(aes(x = date, y = pm10, color = treated_ITS)) +
  facet_wrap(~city) +
  scale_color_discrete(na.value = "grey") +
  labs(
    title = "Treatment assignation for ITS", 
    x = "Date", 
    y = "PM10 concentration level (in mug/m3)"
  )
...
test_treatment_data %>% 
  ggplot() +
  geom_point(aes(x = date, y = pm10, color = treated_RDD)) +
  facet_wrap(~city) +
  scale_color_discrete(na.value = "grey") +
  labs(
    title = "Treatment assignation for RDD", 
    x = "Date", 
    y = "PM10 concentration level (in mug/m3)"
  )
...
test_treatment_data %>% 
  ggplot() +
  geom_point(aes(x = date, y = pm10, color = treated_DID)) +
  facet_wrap(~city) +
  scale_color_discrete(na.value = "grey") +
  labs(
    title = "Treatment assignation for DID", 
    x = "Date", 
    y = "PM10 concentration level (in mug/m3)"
  )
```
...
compute_simulation <- function(data, n_days_study = 200, p_treat = 0.5, treatment_type = "RCT", percent_effect_size = 0.5, formula = "deaths_all_causes ~ treated") {
  ...
  sim_output <- sim_data %>%
    estimate_model(formula = str_c("yobs ~ ", indep_var)) %>%
    mutate(true_effect = mean(sim_data$y1 - sim_data$y0, na.rm = TRUE))
  
  return(sim_output)
} 

test <- compute_simulation(total_data %>% filter(!is.na(deaths_all_causes)), formula = "deaths_all_causes ~ treated")
````
...
names(sim_param) <- str_remove_all(names(sim_param), "vect_")
```
