...
test_treatment_data <- total_data %>% 
  mutate(study_period = draw_study_period(date, 500)) %>% 
    ...
    select(-study_period) %>%
    mutate(
      treated_random_days = draw_treated(., 0.2, "random_days"),
...
sim_param <- sim_param_unique %>%
  ...
  mutate(
    formula = case_when(
      quasi_exp == "local" ~ str_replace_all(formula, "treated", "treated + post + city_treated"),
      str_starts(quasi_exp, "national") ~ str_replace_all(
        formula, "treated", "treated + t + t_post"),
