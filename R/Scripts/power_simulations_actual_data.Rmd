...
# create_y1(total_data, quasi_exp = "OLS_co", formula = "log(resp_total) ~ co + temperature | city + day_of_week + month^year") %>% select(resp_total, y1)
```
...
estimate_model <- function(data, formula) {
  ...
  param_of_interest <- ifelse(
    sum(str_detect("treated", paste(actual_fml))), 
    "treatedTRUE", 
    "co"
  )
...
compute_simulation <- function(data,
  ...
  dep_var <- ifelse(
    paste(fml[[2]][2]) != "NULL", 
    paste(fml[[2]][2]), #not to select the log
    paste(fml[[2]])
  )
  ...
  sim_data <- data %>%
    ...
    create_y1(percent_effect_size, quasi_exp, fml) %>% 
    mutate(
