...
compute_simulation <- function(data, n_days_study = 200, p_treat = 0.5, treatment_type = "RCT", percent_effect_size = 0.5, formula = deaths_all_causes ~ treated) {
  ...
  if(treatment_type == "DID") {
    sim_data <- sim_data %>% 
      group_by(city) %>% 
      mutate(city_treated =  max(treated), NA) %>% 
      ungroup() %>% 
      group_by(date) %>% 
      mutate(post = max(treated), NA) %>%
      ungroup() 
  } 
  ...
  if(treatment_type == "ITS") {
    sim_data <- sim_data %>% 
      ...
      mutate(
        ...
        t_post = max(0, as.numeric(date) - as.numeric(date[treated == TRUE]))
      ) %>% 
      ungroup()
  } 

  if(str_starts(treatment_type, "RDD")) {
    pollutant <- str_extract(treatment_type, "(?<=_).+")
    update(fml, . ~ . + pollutant)
    #update formulma
  }
