...
- For correlation of the allocation with covariates, we can consider that it is correlated with different covariates and in different ways
- For heterogeneous effect of the treatment, it can vary with time, indviduals and covariates
...
```{r}
draw_treated_days <- function(data, p_treat = 0.5, treatment_type = "RDD_pm10") {
...
create_y1 <- function(dep_var, percent_effect_size = 0.5, treatment_type = "RCT", pollutant = NULL) {
  ...
  if (str_starts("RDD_gr", ("RDD|RCT|DID|ITS"))) {
...
The model should be specified in a three part character-formula as follows: "y ~ x | fixed effects | cluster". If no fi
...
# test <- total_data %>%
#   compute_simulation(formula = "deaths_all_causes ~ treated")
````
...
We can then run the simulations. As mentioned, we want to test different sets of parameters. For each set we want to run many iterations of the simulation, `n_iter`. We thus create a table displaying in each row a set of parameters we want to have a simulation for, `sim_param`. Each set of parameter is appears `n_iter` times in `sim_param` to map over the set of parameters.

To build `sim_param`, we first define the different values we want to consider for each parameter. We also build a vector of simulation ids (that we drop afterwards). Finally, we cross that with `vect_treatment_type` to get all sets of parameters for each type of treatment.

```{r}
# vect_n_days_study <- c(100, 250, 500, 750, 1000)
vect_n_days_study <- 200
vect_p_treat <- c(0.1, 0.5)
vect_treatment_type <- c("RCT", "ITS", "RDD_pm10")
# vect_percent_effect_size <- c(0.1, 0.5, 1)
vect_percent_effect_size <- 0.5
vect_formula <- c("deaths_all_causes ~ treated")
n_iter <- 3
...
#Set manually the list of parameters one wants to test

sim_param <- read_csv( "../Outputs/sim_param.csv")

sim_param <- sim_param %>% 
  crossing(
    vect_treatment_type, 
    vect_sim_id = 1:n_iter
  ) %>% 
  select(-vect_sim_id) %>% 
  rename_with(~ str_remove_all(., "vect_"))
```
...
summarise_simulations <- function(data) {
  ...
  data %>% 
    ...
    summarise(
      ...
      bias = mean(abs((estimate - true_effect)/true_effect), na.rm = TRUE)*100,
