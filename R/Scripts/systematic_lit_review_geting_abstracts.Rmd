...
- Effect: the first number preceding by less than 17 characters a similar string describing a confidence interval (we do not consider "95%").
...
Once the effects and CI are identified, some wrangling is necessary in order to get the data into a usable format. We also choose to **drop effects which do not fall into the CI** (62 estimates) in order to get rid off most of the poorly detected effects-CIs. This leads us to also drop non significant estimates. We could also only drop estimates which are not in an interval twice as big as the CI to be more conservative.
