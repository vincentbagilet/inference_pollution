...
get_abstracts <- function(doi) {
  ...
  for (i in 0:number_periods) {
    ...
    if (inherits(possible_error, "error")) {
      warning(paste("The abstracts for the following articles could not be downloaded:", doi_period))
      next
    } else {
       abs <- abs %>%
        rbind(abs_period)
...
effects_CI <- abstracts %>% 
  ...
  filter(contains_CI) %>%
  mutate(
    CI = str_extract_all(sentence, "((?<=(95\\s?%|\\bCI\\b|\\b(c|C)onfidence (i|I)nterval\\b)[^\\d]{0,4})(-\\s?|−\\s?)?[\\d\\.]{1,7}[–\\s:;,%\\-to\\‐]{1,5}(-\\s?|−\\s?)?[\\d\\.]{1,7})|(?<=(\\(|\\[))[\\d-]{1,4}\\.?\\d{0,5}[–\\s:;,%-to‐]{1,5}[\\d-]{1,4}\\.?\\d{0,5}(?=%?[\\)\\];])"),
    effect = str_extract_all(sentence, "(-\\s?|−\\s?)?[\\d\\.]{1,7}(?=[^\\d.]{0,7}([^\\.\\d]\\b95\\s?%|\\bCI|\\b(c|C)onfidence (i|I)nterval\\b))(?<!\\b95)|(-\\s?|−\\s?)?[\\d\\.]{1,7}(?=%?\\s?(\\(|\\[)[\\d-]{1,4}\\.?\\d{0,5}[–\\s:;,%-to‐]{1,5}[\\d-]{1,4}\\.?\\d{0,5}(?=%?[\\)\\];]))(?<![^\\.\\d]95)")
  ) 
```
