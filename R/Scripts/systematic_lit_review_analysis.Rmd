...
We retrieved estimates and confidence intervals of articles in the literature of interest in [another document](systematic_lit_review_geting_abstracts.html). Before diving into the power analysis itself, we look at the characteristics of the articles considered.
...
This query returns `r nrow(abstracts_and_metadata)` articles. Based on the abstracts, we can briefly explore the main (unsurprising) themes of the articles:
...
Out of the `r nrow(abstracts_and_metadata)` abstracts returned by the query, `r effect_detected %>% filter(contains_CI) %>% nrow()` mention the terms "confidence interval", "CI", "Credible Interval", "95%", etc. We virtually covered all possible spelling of these different terms.
...
This corresponds **to `r nrow(estimates)` valid effects** and associated confidence intervals.
...
effect_detected %>% 
  ...
  datatable(colnames = c("Authors", "Publication date", "Title", "Journal"))
```


## Comparison between abstracts for which an effect was retrieved or not
...
Even though there are slightly more recent (2010-2020) articles for which effects are retrieved, the difference does not seem to be substantial. The first article for which an effect is detected was published in `r year_first_article`. Not many articles were published on this topic before this date. We only found `r effect_detected %>% filter(year(pub_date) < year_first_article) %>% count() %>% .$n` articles published before `r year_first_article`. In most places, air pollution has only been measured consistently since the 1990s.
...
effect_detected %>% 
  ...
  mutate(has_effect = ifelse(has_effect, "Effect detected", "Effect not detected")) %>% 
  ggplot() +
  geom_bar(aes(x = subpop, fill = has_effect), position = "dodge") +
  labs(
    x = NULL,
    y = "Number of articles",
    title = "Number of articles studing a given outcome",
...
estimates_stats %>% 
  ...
  kable(col.names = c("At least one significant estimates", "Number of articles", "Proportion"))
```
