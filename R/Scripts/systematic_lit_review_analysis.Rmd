...
<!--     nb_cities = str_extract_all(abstract, "(?<!more\\sthan)\\d+(?=\\s?(c|C)it(y|ies))"), -->
<!--     abstract_ascii = stri_trans_general(abstract,"Latin-ASCII") -->
<!--     # abstract_ascii = str_remove_all(abstract_ascii, "-"), -->
<!--   ) %>%  -->
<!--   unnest(nb_cities, keep_empty = TRUE) %>%  -->
<!--   group_by(doi) %>%  -->
<!--   mutate(nb_cities = max(nb_cities)) %>% -->
<!--   ungroup() %>%  -->
<!--   distinct()  -->
<!--   # group_by(doi) %>%  -->
<!--   # mutate( -->
<!--   #   nb_names_cities = sum(str_detect(abstract_ascii, worldcities_clean[["city_regex"]])) -->
<!--   # ) %>%  -->
<!--   # ungroup() -->
