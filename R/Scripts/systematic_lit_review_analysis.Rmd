...
Key findings and graphs are presented in the [conclusion](\@concl)) section of the present document. This section has been built to be read and understood on its own. Readers who are only interested in the key findings are invited to jump straight to this section.  
...
We find that, **even if the measured effect was close to the true effect, for a good chunk of articles, there  would be a substantial risk of type M error**. That means that, replicating each of these studies would yield, on average, to smaller effect than the reported one. 
...
### Evolution of power with publication year
...
To sum up, the bulk of the estimates in this literature does not seem to suffer from low power and type M error issues. For most estimates, type S error does not seem to be an important issue in this literature. Yet, even if the measured effect was close to the true effect, a non negligible chunk of articles would display low power and a substantial risk of type M error. The power of a large chunk of estimates, at least one quarter of significant ones, might be concerning low. This low power is associated with non negligible type M error (median of 1.25). Part of the estimates are likely to be overestimated. 

We thus need to investigate further into the sources of this heterogeneity.

# Analysis of the sources of heterogeneity

In the previous section, we noticed that, while a good chunk of the literature seems not to suffer too much from power issues, a non negligible number of studies does. In this section, we thus investigate whether we can find clear sources for this heterogeneity. To simplify and clarify our analysis, we consider that an estimate has low power if its computed power is lower than 80% if the true effect is 3/4 of the measured effect. These numbers are arbitrary. 80% is the threshold usually used in power analyses but 3/4 is arbitrary and could be changed easily in a robustness check. Following this criterion, the number and proportion of estimates with low power is as follows:
...
First, we look into the distribution of publication dates.
...
**The proportion of articles with low power does not seem to have drastically evolved in time**. One may have expected that this proportion would have decreased in time as researchers have access to more and more data and computational power and as estimation methods improve. 
