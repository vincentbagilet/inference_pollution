...
abstracts_CI %>% 
  ...
  mutate(nb_journal = n()) %>% 
  ungroup() %>% 
  mutate(journal = reorder(journal, nb_journal)) %>% 
  filter(nb_journal > 15) %>% 
  ggplot() +
  geom_bar(aes(journal, fill = has_effect)) +
  coord_flip() +
  labs(x = NULL, y = "Number of articles", title = "Main journals in which effeccts have been published", subtitle = "Comparison between effects detected or not") +
  scale_fill_discrete(name = "")
```
...
theme_effect <- abstracts_CI %>%
  ...
  ungroup() %>% 
  select(word, has_effect, n_articles_word) %>% 
  distinct() %>% 
  group_by(word) %>% 
...
theme_effect %>% 
  ...
  coord_flip() +
  labs(x = NULL, y = "Number of abstracts containing a given word", title = "Words appearing in the more abstracts", subtitle = "Comparison between articles with and without detected effect") +
  scale_fill_discrete(name = "")
```
...
abstracts_CI %>% 
  ...
  mutate(has_effect = ifelse(has_effect, "Effect detected", "Effect not detected")) %>% 
  group_by(pollutant) %>% 
  mutate(nb_pollutant = n()) %>% 
  ungroup() %>% 
  mutate(pollutant = reorder(pollutant, nb_pollutant)) %>% 
  ggplot() +
  geom_bar(aes(pollutant, fill = has_effect)) +
  coord_flip() +
...
abstracts_CI %>% 
  ...
  mutate(has_effect = ifelse(has_effect, "Effect detected", "Effect not detected")) %>% 
  ggplot() +
  geom_bar(aes(pollutant, fill = has_effect), position = "fill") +
  coord_flip() +
  labs(x = NULL, y = "Number of articles", title = "Number of articles studing a given pollutant", subtitle = "Comparison between articles for which an effect was detected or not") +
  scale_fill_discrete(name = "")
```
...
abstracts_CI %>% 
  ...
  mutate(has_effect = ifelse(has_effect, "Effect detected", "Effect not detected")) %>% 
  ggplot() +
  geom_bar(aes(x = outcome, fill = has_effect), position = "stack") +
  labs(x = NULL, y = "Number of articles", title = "Number of articles studing a given outcome", subtitle = "Comparison between articles for which an effect was detected or not") +
  scale_fill_discrete(name = "") 
```
...
quantile(articles_analysis$signal_noise, seq(0, 1, 0.1)) %>% 
  tidy() %>% 
  kable(col.names = c("Signal to noise ratio quantile", "Signal to noise ratio"))
```
...
A clearer way to look at the distribution of power, type M and type S errors across studies is to look at their ECDF. 
...
articles_retro %>% 
  ...
  stat_ecdf(geom = "line") +
  facet_wrap(~ prop_true_effect) +
  labs(title = "Cumulative distribution of power in the literature", subtitle = "If the magnitude of true effect is a fraction of the measured effect", x = "Power", y = "Proportion of estimates with a smaller power")
...
articles_retro %>% 
  ...
  # coord_flip() +
  facet_wrap(~ prop_true_effect) +
  labs(title = "Cumulative distribution of power in the literature", subtitle = "If the magnitude of true effect is a fraction of the measured effect", x = "Type M error", y = "Proportion of estimates with a smaller power") 
...
articles_retro %>% 
  ...
  # coord_flip() +
  facet_wrap(~ prop_true_effect) +
  labs(title = "Cumulative distribution of power in the literature", subtitle = "If the magnitude of true effect is a fraction of the measured effect", x = "Type S error", y = "Proportion of estimates with a smaller power") 
```

We notice that more than 50% of studies would be underpowered at the conventional 80% level if we considered that the true effect was half the measured effect.
...
We notice that, on average in the literature, the treatment effects are overestimated, even for large values of the true effect. This result might be linked to some outliers. We therefore look into the evolution of the distribution with the proportion of effect size.

```{r fig.asp=1}
articles_retro %>% 
  ggplot(aes(x = power, y = factor(prop_true_effect))) +
  geom_density_ridges(color = "#00313C", fill = "#00313C", alpha = 0.3) +
  labs(title = "Evolution of distrubtion of power as a function of the 'true effect'", x = "Power", y = "True effect as a proportion of the measured effect") 

articles_retro %>% 
  filter(typeM < 5) %>% 
  ggplot(aes(x = typeM, y = factor(prop_true_effect))) +
  geom_density_ridges(color = "#00313C", fill = "#00313C", alpha = 0.3) +
  labs(title = "Evolution of distrubtion of type M error as a function of the 'true effect'", x = "Type M error", y = "True effect as a proportion of the measured effect")

articles_retro %>% 
  ggplot(aes(x = typeS, y = factor(prop_true_effect))) +
  geom_density_ridges(color = "#00313C", fill = "#00313C", alpha = 0.3) +
  labs(title = "Evolution of distrubtion of type M error as a function of the 'true effect'", x = "Type S error", y = "True effect as a proportion of the measured effect")
```
We also look into the evolution of the median effect with true effect size.
...
```{r}
colors_mediocre <- mediocrethemes::colors_table[[which(mediocrethemes::colors_table$color == "autumn"), "vector"]] %>% 
  strsplit(split=", ") %>% 
  unlist()
...
articles_low_adequate %>% 
  ...
  filter(nb_journal > 15) %>% 
  ggplot() +
  geom_bar(aes(journal, fill = low_power)) +
  coord_flip() +
  labs(x = NULL, y = "Number of articles", title = "Main journals in which effects have been published", subtitle = "Comparison between effects that have adequate power or not") +
  scale_fill_manual(values = colors_low_power, name = "")
```
...
articles_low_adequate %>% 
  ggplot() + 
  geom_histogram(aes(x = pub_date, fill = low_power), position = position_fill(), bins = 10)
```
...
articles_low_adequate %>% 
  ...
  mutate(pollutant = reorder(pollutant, nb_pollutant)) %>% 
  ggplot() +
  geom_bar(aes(pollutant, fill = low_power)) +
  coord_flip() +
  labs(x = NULL, y = "Number of articles", title = "Number of articles studing a given pollutant", subtitle = "Comparison between articles with low and adequate power") +
  scale_fill_manual(values = colors_low_power, name = "")
```
...
articles_low_adequate %>% 
  ...
  # mutate(nb_outcome = n()) %>% 
  # ungroup() %>% 
  # mutate(outcome = reorder(outcome, nb_outcome)) %>% 
  ggplot() +
  geom_bar(aes(outcome, fill = low_power), position = position_fill()) +
  # coord_flip() +
  labs(x = NULL, y = "Number of articles", title = "Number of articles considering a given outcome", subtitle = "Comparison between articles with low and adequate power") +
  scale_fill_manual(values = colors_low_power, name = "")
```
...
more <- abstracts_more_info %>% 
  mutate(
    length_study = as.numeric(str_extract(end_obs, "\\d+")) - as.numeric(str_extract(begin_obs, "\\d+"))
  ) %>% 
  filter(end_obs <= 2021) %>% 
  filter(length_study >= 0)
