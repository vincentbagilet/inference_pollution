...
```{r}
effects_and_CI <- detected_effects %>% 
...
saveRDS(effects_and_CI, "../Outputs/effects_and_CI.RDS")
```

The `effects_and_CI` data frame displays, in each row, a point estimate and the lower and upper band of the CI along with the DOI of the article from which this estimate is extracted. We retrieved `r nrow(effects_and_CI)` valid point estimates and associated confidence intervals. We analyze them further in another document.
...
saveRDS(articles_metadata, "../Outputs/articles_metadata.RDS")
```
