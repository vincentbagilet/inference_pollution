...
It might also be interesting to information about the type of pollutant considered in the study, the study period, the number of observations or p-values and the type of outcome studied (mortality, emergency admissions, stroke, cerebrovascular or cardiovascular diseases). We also use regex to recover this information.
...
abstracts_length_study <- abstracts_more_info %>%
  mutate(
...
abstracts_number_cities <- abstracts_numbers %>% 
  ...
  mutate(nb_many_cities = as.numeric(nb_many_cities)) %>% 
  group_by(doi) %>%
  ...
  group_by(doi) %>%
  mutate(
    nb_names_cities = str_extract_all(abstract_ascii, worldcities_large[["city_regex"]]) %>% 
      as_vector() %>% 
      unique() %>% 
      length()
  ) %>%
  ...
  group_by(doi) %>% 
  mutate(
    nb_cities = max(nb_many_cities, nb_names_cities, na.rm = TRUE),
    nb_cities = ifelse(nb_cities == 0, NA, nb_cities)
  ) %>% 
  ungroup() %>% 
  select(doi, nb_cities) %>% 
  distinct()
```

Finally, we combine these two information to compute the number of observations. We also retrieve the number of observations directly from the abstract when it is obviously stated.

```{r}
abstracts_more_info <- abstracts_more_info %>% 
  right_join(abstracts_length_study) %>% 
  right_join(abstracts_number_cities) %>% 
  mutate(
    n_obs_stated = str_extract_all(
      abstract, 
      "(?<=(\\bn\\s?= ))[,\\d\\s]{1,10}|[,\\d\\s]{1,8}(?=(\\sobservations))"
    )
  ) %>% 
  unnest(n_obs_stated, keep_empty = TRUE) %>%
  mutate(
    n_obs_stated = as.numeric(str_remove_all(n_obs_stated, ",")), 
    n_obs_computed = nb_cities*length_study
  )  
  # group_by(doi) %>% 
  # mutate(
  #   nb_obs = max(n_obs_stated, n_obs_computed, na.rm = TRUE)
  # ) %>% 
  # ungroup()
```
