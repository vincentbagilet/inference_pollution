...
```{r eval = FALSE}
query <- 
  str_c('TITLE(("air pollution" OR "air quality" OR "particulate matter" OR ozone', 
...
```{r}
scopus_subject_corres <-
  read_csv("../Inputs/scopus_subject_corres.csv") %>%
  rename(
    area_code = Code,
    sub_subject_area = Field, 
    subject_area = `Subject area`
  ) %>%
  drop_na()

scopus_journal_subsubject <- read_csv("../Inputs/scopus_journal_subsubject.csv") %>% 
  rename(
    journal = Title,
    sub_subject_area = `Scopus Sub-Subject Area`,
    area_code = `Scopus ASJC Code (Sub-subject Area)`
  ) %>% 
  select(journal, sub_subject_area, area_code)

journal_subject <- scopus_journal_subsubject %>% 
  left_join(scopus_subject_corres, by = c("area_code", "sub_subject_area")) %>% 
  select(-area_code) %>% 
  mutate(
...
articles_journal_subject <- abstracts %>% 
  ...
  select(doi, subject_area, sub_subject_area) %>% 
  distinct() %>% 
  nest(subject_area = c(subject_area), sub_subject_area = c(sub_subject_area)) 
```
