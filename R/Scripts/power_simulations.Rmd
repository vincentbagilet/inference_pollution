...
- For the IV, we use the same method as for the OLS but previously, we modified the value of pollutant concentration through the instrument. The instrument is binary and affects pollutant concentration: $Poll_{ct}^{fake} = Poll_{ct} + \alpha T_{ct} + \epsilon_{ct}$, where $T_{ct}$ is the treatment dummy, $\alpha$ the treatment "intensity" (the `iv_intensity` in our function) and $\epsilon \sim \mathcal{N}(0, 0.1)$ a noise.
