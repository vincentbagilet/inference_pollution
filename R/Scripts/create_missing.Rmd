...
```{r}
crea_unique_length <- function(prop_missing, length_period) {
...
one_length <- data_daily %>%  
  ...
  mutate(missing_crea = crea_unique_length(0.2, 4)) %>% 
  ungroup()
```
...
```{r}
crea_various_length <- function(prop_missing, df_prop) {
  missing <- rep(0, n())
  ...
  if(null_list[1]) {#to avoid issue when df_prop is a NULL list (due to merging)
    for (i in rev(1:nrow(df_prop_uniq))){
      missing <- missing +
        crea_unique_length(prop_missing*df_prop_uniq[[i, "prop"]],
...
missing_per_city_poll <- data_daily %>% 
  ...
  group_by(city, pollutant) %>% 
  mutate(missing_crea = crea_various_length(0.1, length_prop)) %>% 
  select(-length_prop) %>% 
  ungroup()
...
Due to their large share of missing values, we do not consider `uv_radiation`, `insolation_duration` nor `global_radiation` as covariates. Thus, we consider the following covariates:

```{r}
covar_weather <- data_daily %>% 
  select(rainfall_height:relative_humidity) %>% 
  names()

covar_date <- data_daily %>% 
...
We also consider lagged and squared values of the weather variables.

```{r}
identity_weather <- str_c(covar_weather, collapse = " + ")
lagged_weather <- str_c("lag(", covar_weather, ")", collapse = " + ")
squared_weather <- str_c("I(", covar_weather, "^2)", collapse = " + ")
identity_date <- str_c(covar_date, " + month*year", collapse = " + ")

covar <- str_c(identity_weather, lagged_weather, squared_weather, identity_date, sep = " + ")
```
...
mod <- glm(data = data_daily, 
           as.formula(str_c("missing ~ ", covar)),
           family = binomial(link = "probit"))
```

Once we have estimated the model, we can predict whether an observation will be missing or not and thus create a "false" missingness observations.

```{r}
pred <- predict(mod, type = "response")

missing_covariate <- mod$model %>% 
  as.tibble() %>% 
...
We then quickly look into the performance of our prediction procedure.

```{r}
missing_covariate %>% 
  count(missing_crea, missing) %>% 
  mutate(prop = n/sum(n)) %>% 
  kable()
```
