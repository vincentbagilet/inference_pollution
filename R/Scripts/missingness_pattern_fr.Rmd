...
if (hourly_daily == "hourly") {
  ...
  names_prop <- data %>%
    select(-(pollutant:site), concentration, -(month:day_in_week), -date, -missing) %>% 
    names()
}
...
if (hourly_daily == "hourly") {
  ...
  covariate_names <- data %>% 
    select(city, pollutant, rainfall_height:day_in_week, -wind_direction, -wind_direction_max) %>%
    names()
}

purrr::map(covariate_names, hist_missing_by, df = data_first_missing, variable_missing = "concentration", bins = 10, facet_var = NA, y_axis_to = if (hourly_daily == "hourly") {0.05} else {0.02})
```
