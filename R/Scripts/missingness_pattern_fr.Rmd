---
title: "Analysis of missing data patterns (in hourly data set)"
author:
...
if (hourly_daily == "hourly") {
  ...
  covariate_names <- data %>% 
    select(city, pollutant, elevation_weather_station, rainfall_height:day_in_week) %>%
    names()
}


purrr::map(covariate_names, hist_missing_by, df = data_first_missing, variable_missing = "concentration", bins = 10, facet_var = NA, y_axis_to = 0.05)
```
...
```{r heatmap, fig.asp = if (hourly_daily == "hourly") {2.4} else {0.7}}
...
graph_distrib_length <- function(df, density, bins_adjust) {
  ...
  if (density) {
    ...
    facet_wrap(~pollutant) +
    labs(title = "Distribution of the length of missingness periods", subtitle = paste("Comparison across pollutants - One observation per", per, sep = " "), x = "Number of consecutive hours of missing data") 
