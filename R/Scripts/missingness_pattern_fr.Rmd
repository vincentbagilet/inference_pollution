...
```{r}
graph_desity_covariate <- function(df, covar, facet_var = NA) {
...
map(continuous_var_names, graph_desity_covariate, df = data, facet_var = NA)
```
...
One may also be interested in looking at these distributions by pollutant.

```{r}
map(continuous_var_names, graph_desity_covariate, df = data, facet_var = "pollutant")
...
map(c("concentration", continuous_var_names), graph_desity_covariate, df = first_missing, facet_var = NA)
map(c("concentration", continuous_var_names), graph_desity_covariate, df = first_missing, facet_var = "pollutant")
...
```{r fig.asp = 3}
graph_desity_covariate(df = filter(first_missing, concentration < 100) ,covar = "concentration") + facet_wrap(~ pollutant, ncol = 1)
```
