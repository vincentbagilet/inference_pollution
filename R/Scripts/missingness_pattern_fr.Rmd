...
data <- readRDS("../Outputs/data_imputation.RDS")
...
data %>%  
  ...
  filter(year != 2014) %>% 
  hist_missing_by(variable_missing = "concentration", grouping_var = "city") +
  coord_flip()
```
...
```{r}
data %>%
  line_missing_by(variable_missing = "concentration", grouping_var = "temperature")
```
...
```{r}
data %>% 
  line_missing_by(variable_missing = "concentration", grouping_var = "wind_speed")
```

```{r}
data %>% 
  line_missing_by(variable_missing = "concentration", grouping_var = "wind_speed_max")
```

```{r}
data %>% 
  line_missing_by(variable_missing = "concentration", grouping_var = "wind_direction") +
  coord_polar()
```
...
```{r}
data %>% 
  line_missing_by(variable_missing = "concentration", grouping_var = "wind_direction_max") +
  coord_polar()
```
...
```{r}
data %>% 
  line_missing_by(variable_missing = "concentration", grouping_var = "rainfall_height") 
```

```{r}
data %>% 
  line_missing_by(variable_missing = "concentration", grouping_var = "relative_humidity") 
```
