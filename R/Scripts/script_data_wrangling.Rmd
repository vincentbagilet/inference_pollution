...
data_daily <- data_imputation %>%  
  ...
  select(-site:-site_area, -wind_speed_max, -wind_direction_max, -holiday_zone) %>% 
  rename(
    ws = wind_speed,
    wd = wind_direction
  ) %>% #need to rename these for timeAverage
  ...
  mutate(
    ...
    holiday_zone = ifelse(city %in% c("Caen", "Clermont-Ferrand", "Grenoble", "Lyon", "Montpellier", "Nancy", "Metz", "Nantes", "Rennes", "Toulouse"), "a",
                  ifelse(city %in% c("Aix", "Marseille", "Amiens", "Besancon", "Dijon", "Lille", "Limoges", "Le Havre", "Nice", "Orleans", "Tours", "Poitiers", "Reims", "Rouen", "Strasbourg"), "b", 
                  ifelse(city %in% c("Bordeaux", "Creteil", "Paris", "Versailles"), "c", NA))) #read this because timeAverage does not work well with categorical variables
  ) %>% 
  select(date, city, pollutant, concentration, site, everything()) %>%  #to have the same order of variables in both data sets
  filter(!is.na(elevation_weather_station)) #if want to drop dates*pollutant*city where no station open in the city

saveRDS(data_daily, "../Outputs/data_daily.RDS")
```

Note that for some cities x pollutants (*eg* SO2 in Nancy), for a given period, there are no stations open and therefore no measurements. Illustration of the issue:
...
data_imputation %>% 
  ...
  summarise(mi = min(date), ma = max(date), .groups = "drop")
```
One can either consider that the data for this city x period is missing (and fill the corresponding rows with `NA` for concentration) or remove completely the corresponding observations. This affects the share of missing concentration values. We choose the later solution, assuming that the data is not actually missing for these observations.
