...
```{r}
clean_metadata_weather <- metadata_weather_raw %>% 
  ...
  mutate(
    ...
    city = recode(city, "Marignane" = "Marseille", "Octeville" = "Le Havre")
  )
...
data_imputation <- clean_pollution_data %>% 
  ...
  left_join(clean_weather_data, by = c("date", "weather_station_id")) %>% # distinct to remove when error with Toulouse fixed
  select(-weather_station_id) %>% 
  select(date, city, pollutant, concentration, everything()) %>% 
  mutate(date_day = lubridate::date(date)) %>% 
  mutate(
    holiday_zone = ifelse(city %in% c("Caen", "Clermont", "Grenoble", "Lyon", "Montpellier", "Nancy", "Metz", "Nantes", "Rennes", "Toulouse"), "a",
                  ifelse(city %in% c("Aix", "Marseille", "Amiens", "Besancon", "Dijon", "Lille", "Limoges", "Nice", "Orleans", "Tours", "Poitiers", "Reims", "Rouen", "Strasbourg"), "b", 
