...
data_deryugina_2019_iv <-
  retro_design_closed_form(as.list(seq(0.01, 0.7, by = 0.001)), 0.061) %>%
  unnest() %>%
...
Suppose that the true effect of the increase in PM$_{2.5}$ was 0.095 additional deaths per million elderly individuals - the estimate found with a naive multivariate model. The statistical power would be `r round(retro_design_closed_form(0.095, 0.061)$power*100, 0)`% and the overestimation factor would be equal to `r round(retro_design_closed_form(0.095, 0.061)$type_m, 1)`. The type M error would be worrying for the instrumental variable strategy if the true effect size is the estimate obtained with the standard multivariate model. However, if the true effect size was equal to the lower bound of the 95% confidence interval of the 2SLS estimate, the `r round(retro_design_closed_form(0.57044, 0.061)$type_m, 1)` would be equal to 1: the study would not suffer from a type M error.
