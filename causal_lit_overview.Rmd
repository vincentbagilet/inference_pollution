...
We load the literature review data and export it to `.csv`. You can download it [here](https://github.com/vincentbagilet/inference_pollution/tree/master/shared).

```{r load_data, echo=TRUE, code_folding=FALSE}
# load literature review data
data_lit_causal <- readRDS(here("data", "lit_causal", "data_lit_causal.rds"))
```

### Description of Coded Variables

We retrieved data `r nrow(data_lit_causal)` estimates from `r data_lit_causal %>% select(paper_label) %>% distinct() %>% nrow()` articles. For each paper, we coded 25 variables:

* `paper_label`: the first author and publication date of the article. This is the identifier of a study.
* `paper_estimate_id`: this is the unique identifier of a result. The median number of results reported by studies is `r median(data_lit_causal %>% group_by(paper_label) %>% summarise(n = n()) %>% pull(n))`.
* `url`: the internet link of the publication.
* `publication_year`: year of publication. Papers we consider were published between `r min(data_lit_causal$publication_year)` and `r max(data_lit_causal$publication_year)`.
* `field`: whether the variable was published in economics or in epidemiology. `r data_lit_causal %>% select(paper_label, field) %>% distinct() %>% summarize(round(mean(field=="economics")*100,0))` of studies are published in economics journals, the rest being published in epidemiology/public health journals.
* `context`: the location of the study.
* `empirical_strategy`: the empirical strategy used by researchers ("conventional time series model", "reduced-form", "difference-in-differences, "instrumental variable", etc...). We display below the 5 most used empirical strategies for our `r nrow(data_lit_causal)` estimates:
...
* `outcome_type`: whether the health outcome is a mortality or morbidity outcome. `r round(sum(data_lit_causal$outcome_type=="mortality")/nrow(data_lit_causal)*100, 0)`% of outcomes are related to mortality.
...
* `independent_variable_dummy`: indicates whether the independent variable is binary. `r round(sum(data_lit_causal$independent_variable_dummy==1)/nrow(data_lit_causal)*100, 0)`% of estimates are for a binary treatment.
* `is_standardized`: indicates whether the estimates have already been standardized. `r round(sum(data_lit_causal$is_standardized==1)/nrow(data_lit_causal)*100, 0)`% of estimates are already standardized.
* `log_outcome`: is the estimate expressed in relative term. `r round(sum(data_lit_causal$log_outcome==1)/nrow(data_lit_causal)*100, 0)`% of estimates represent relative increase in the health outcome.
...
* `open_data`: whether the data are available to reproduce the findings of the study. Only `r round(sum(data_lit_causal$open_data==1)/nrow(data_lit_causal)*100, 0)`% of results are directly reproducible.

The data set is as follows:

```{r datatable_all_estimates, code_folding=TRUE, layout="l-body-outset"}
data_lit_causal |> 
  DT::datatable(
    filter = 'top', 
    options = list(
...
We are able to standardize the effects of `r round(sum(!is.na(data_lit_causal_std$standardized_estimate))/nrow(data_lit_causal_std)*100, 0)`% of all estimates. We display below summary statistics on the distribution of the standardized effect sizes of causal inference methods:
...
Half of the studies estimate effect sizes below `r round(summary_std[["Median"]], 2)` standard deviation.
...
For half of the studies reporting both an IV estimate and an OLS estimate, the ratio of these respective estimates is larger than `r round(median(data_ratio_ols_iv$ratio), 1)`. The median ratio of the ratio of the standard errors is `r round(median(data_ratio_ols_iv$ratio_se), 1)`.

# Evidence of Publication Bias

### Distribution of z-statistics

We plot the distribution of weighted z-statistics by following [Brodeur et al. (2020)](https://www.aeaweb.org/articles?id=10.1257/aer.20190687) where the weights are equal to the inverse of the number of tests in an article. As in the paper, we assume that the degrees of freedom are large enough so that we can assume that the estimates are normally distributed.

```{r graph_z_statistics}
data_lit_causal_caliper <- data_lit_causal_std %>%
  filter(control_outcome != 1) %>% #remove placebo
  filter(str_detect(empirical_strategy, "conventional time series", negate = TRUE)) %>%
  # mutate(table = stringr::word(source_results, 1, sep = " - ")) %>% 
  add_count(paper_label, name = "n_tests") %>% 
  mutate(
    weight = 1/n_tests,
...
graph_distribution_t <- data_lit_causal_caliper %>%
  ...
  geom_histogram(bins = 50, colour = "white", alpha = 0.6) +
  geom_density(bw = 0.4, fill = NA, linewidth = 0.9) +
  ...
  labs(
    title = "Weighted Distribution of the z-statistics",
...
graph_distrib_t_econ <- data_lit_causal_caliper %>%
  ...
  ggplot(., aes(x = abs_z, y = after_stat(density), weight = weight)) + 
  geom_histogram(bins = 50, colour = "white", alpha = 0.6) +
  geom_density(bw = 0.4, fill = NA, linewidth = 0.9) +
  geom_vline(xintercept = 1.96, linewidth = 0.9, colour = "#f07167") +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 10)) +
...
We then implement formal tests, as in [Brodeur et al. (2020)](https://www.aeaweb.org/articles?id=10.1257/aer.20190687).
Caliper tests themselves follow [Gerber and Malhotra (2008a)](https://www.emerald.com/qjps/article-abstract/3/3/313/1328368/Do-Statistical-Reporting-Standards-Affect-What-Is?redirectedFrom=fulltext). 

The idea behind these tests is to compare the number of test statistics just above and just below a significance threshold. We create a function `simple_caliper_test` that computes the number and share of estimates just above and below (in a certain window) a given significance level, weighted by the inverse of the number of test by article.

#### Counting test statistics

We first count test statistics in bandwidths around significance thresholds:

```{r simple_caliper_test, code_folding=FALSE, echo=TRUE}
simple_caliper_test <- function(df, sign_level, window_width) {
  threshold <- qnorm(1 - sign_level/2)
  
  df_w <- df %>%
    filter((abs_z >= threshold - window_width) & (abs_z <= threshold + window_width)) %>%
    mutate(above = (abs_z >= threshold))
...
caliper_simple_results <- 
  ...
  pmap(simple_caliper_test, df = data_lit_causal_caliper) %>%
  list_rbind()
...
```{r simple_caliper_test_graph, fig.asp=1}
caliper_simple_results |>  
  mutate(
    sign_level = as.factor(sign_level),
    window_width = as.factor(window_width)
  ) |> 
  ggplot(aes(x = sign_level, y = window_width, fill = share_above_weighted)) + 
  geom_tile(
    color = "white", linewidth = 2
  ) +
  geom_label(aes(label = round(share_above_weighted, 2))) +
  coord_fixed() + 
  labs(
...
Focusing on the 5% threshold, as in the rest of the paper, the weights just above the threshold are larger than just below, suggesting selection on significance around this threshold. Interestingly, there does not seem to be selection on significance around the 1% threshold. That might be explained by the fact that the main threshold used in the economics literature is the 5% one.

Regardless, considering the relatively small number of observations (`r nrow(data_caliper)` estimates), one might need to analyse these results carefully.

#### Randomization tests

Then, as in Brodeur et al (2020), we test whether the observed test-statistics within each bandwidth could come from a binomial distribution with probability 0.5 to fall above the threshold. 

```{r rand_test}
caliper_simple_results |>
  rowwise() |>
  mutate(p_value_binom_test =
           binom.test(n_above, n_in_window, 0.5, alternative = "greater")$p.value) |>
  select(sign_level, window_width, n_in_window, p_value_binom_test) |>
  kable(
    col.names = c("Significance level", "Window width", "Number of tests", "p-value of binomial test"),
    digits = 3
  ) 
```

We do not find any conclusive evidence but that may partly come from the limited number of tests considered.

#### Caliper test

We then turn to a probit regression in order to be able to compare the likelihood of imbalance across various variables. However, these analyses only have little statistical power, considering the small number of observations.

```{r logit_caliper}
#adding dummy variables to control for
data_caliper_logit <- data_lit_causal_caliper |> 
  mutate(
    is_economics = (field == "economics"),
    is_mortality = (outcome_type == "mortality"),
    recent = (publication_year >= 2019), #median publication year
    is_published = (journal != "working paper"),
    is_iv = (empirical_strategy == "instrumental variable"),
    is_reduced = (empirical_strategy == "reduced-form"),
    is_did = str_detect(empirical_strategy, "difference in differences")
  )

# define function
run_logit_caliper <- function(df, fml_name, sign_level = 0.05, window_width = 0.5) {
  threshold <- qnorm(1 - sign_level/2)
  df_w <- df %>%
    filter(abs_z >= threshold - window_width, abs_z <= threshold + window_width) %>%
    mutate(above = as.logical(abs_z >= threshold))
  
   fml_str <- switch(fml_name,
    base  = "above ~ 1",
    all = "above ~ is_economics + is_mortality + recent + is_published + is_iv + is_reduced + is_did"
  )
...
# apply function to set of parameters

caliper_logit_param <- 
  expand_grid(
    sign_level = c(0.05, 0.1),
    window_width = c(0.20, 0.50),
    fml_name = c("base", "all")
  )

caliper_logit_res <- caliper_logit_param |> 
  pmap(run_logit_caliper, df = data_caliper_logit)

models <- list(
  "Above (signif 5%)" = list(
    "±0.20" = caliper_logit_res[[1]],
    "±0.20" = caliper_logit_res[[2]],
    "±0.50" = caliper_logit_res[[3]],
    "±0.50" = caliper_logit_res[[4]]
  ),
  "Above (signif 10%)" = list(
    "±0.20" = caliper_logit_res[[5]],
    "±0.20" = caliper_logit_res[[6]],
    "±0.50" = caliper_logit_res[[7]],
    "±0.50" = caliper_logit_res[[8]]
  )
)

# names(caliper_logit_res) <- caliper_logit_param |> 
#   mutate(name = paste("Signif:", sign_level, "window:", window_width)) |> 
#   pull(name)
# 
# window_row <- data.frame(
#   term = "Window width",
#   caliper_logit_param %>%
#     mutate(val = paste0("±", window_width)) %>%
#     pull(val) %>%
#     setNames(names(caliper_logit_res)) %>%
#     as.list()
# )
...
modelsummary(
  ...
  coef_rename = c(
    "(Intercept)" = "Intercept",
    "is_economicsTRUE" = "Economics journal",
    "is_mortalityTRUE" = "Morality outcome (dummy)",
    "recentTRUE" = "Published after 2018",
    "is_publishedTRUE" = "Published paper",
    "is_ivTRUE" = "Instrumental variable",
    "is_didTRUE" = "DiD",
    "is_reducedTRUE" = "Reduced form"
  ),
  title = "Caliper Test: Logit Estimates of Excess Mass Above Significance Thresholds",
  notes = paste(
    "Weighted logit.",
    "SEs clustered by article.",
    "The dependent variable equals 1 if |t| is above the threshold."
  )
)

```

Considering the limited number of observations, it is rather difficult to draw clear conclusions. If anything, published papers seems to suffer more from selection on significance than working papers.
...
```{r graph_intro_paper, echo=FALSE, code_folding=FALSE}
...
In this section, we compute the statistical power, the exaggeration ratio and the probability to make a type S error for each study. We rely on the [retrodesign](https://cran.r-project.org/web/packages/retrodesign/vignettes/Intro_To_retrodesign.html) package.
...
```{r data_for_retrodesign, echo=TRUE, code_folding=FALSE}
# test exaggeration and type s errors
data_retro <- data_lit_causal_std %>%
  # drop control outcomes
  filter(control_outcome == 0) %>%
  # drop conventional time series models
  filter(!(
  ...
  # select(paper_label, paper_estimate_id, estimate, standard_error, sample_size) %>%
...
```{r run_retrodesign, echo = TRUE, code_folding=FALSE}
# run retrospective power analysis for decreasing effect sizes
data_retro_frac_tidy <- data_retro |> 
...
data_retro_frac <- data_retro_frac_tidy |> 
  pivot_longer(
...
data_retro_frac_mean <- data_retro_frac %>%
...
graph_retrodesign_causal_inf <- data_retro_frac %>%
...
### Effect Size and Exaggeration

We now explore the relationship between our power metrics and the number of observations, for an hypothetical effect equal to half the point estimate.

```{r exagg_sample_size, fig.asp=0.8}
data_retro_frac |> 
  filter(percentage == 0.5) |> 
  # filter(str_detect(metric, "Exaggeration")) |> 
  ...
  labs(
    title = "Relationship between the metrics of interest and sample size",
    x = "Sample Size",
    y = NULL
  )
```

We also run a quick and simple regression

```{r reg_sample_size}
reg <- 
  feols(
    data = data_retro_frac_tidy, 
    c(power, type_m) ~ log(sample_size), vcov = ~ paper_label
  )

modelsummary(reg, gof_omit = "IC|Adj|Log")
```
...
The median statistical power is `r summary_retro_iv$median[2]`% and the median exaggeration ratio is `r summary_retro_iv$median[1]`. `r round(prop_adequate*100, 2)`% of IV designs have adequate power (*i.e.* greater than 80%).

We also explore how this evolves with the number of observation.

```{r exagg_sample_size_iv, fig.asp=0.8}
data_retro_iv |> 
  # filter(percentage == 0.5) |> 
  # filter(str_detect(metric, "Exaggeration")) |> 
  ggplot(aes(x = sample_size, y = value)) + 
...
```{r reg_sample_size_iv}
reg_iv <- data_retro_iv |> 
...
modelsummary(reg_iv, gof_omit = "IC|Adj|Log")
```


<!-- # Publication bias correction -->

<!-- Then, we implement [Andrews and Kasy (2019)](https://www.aeaweb.org/articles?id=10.1257/aer.20180310)'s correction for publication bias.  -->

<!-- ```{r} -->
<!-- library(MetaStudies) -->

<!-- df <- data_lit_causal_std %>% -->
<!--   filter(control_outcome != 1) |>  -->
<!--   # filter(standardized_estimate != 0, standardized_standard_error != 0) |>  -->
<!--   filter(estimate != 0, standard_error != 0) |> -->
<!--   mutate(z_stat = estimate / standard_error) -->


<!-- # Run Andrews-Kasy -->
<!-- ms <- metastudies_estimation( -->
<!--   X         = df$estimate, -->
<!--   sigma     = df$standard_error, -->
<!--   cutoffs   = c(1.645, 1.96, 2.576), -->
<!--   symmetric = TRUE, -->
<!--   model     = "t" -->
<!-- ) -->
...
<!-- # Plot: publication probabilities (top) + latent density (bottom) -->
<!-- estimates_plot(ms)  -->

<!--          # Specification check: X-sigma independence in latent distribution -->
...
<!--   janitor::clean_names() |>  -->
<!--   filter(z < 7) |>  -->
<!--   ggplot(aes(x = z, y = pub_prob)) +  -->
<!--   geom_line(linewidth = 1) +  -->
<!--   ylim(c(0, NA)) -->

<!-- # --- 2. Extract the step-function p(·) from the estimation ------------------- -->

<!-- # The estimated relative publication probabilities per |z| bin. -->
<!-- # Last bin (|z| > 2.576) is normalized to 1. -->
<!-- # We read them from est_tab: columns after the latent params are the bins. -->
<!-- latent_cols <- intersect(c("µ", "sigma", "t", "df", "τ"), colnames(ms$est_tab)) -->
<!-- pub_cols    <- setdiff(colnames(ms$est_tab), latent_cols) -->
<!-- pub_weights <- ms$est_tab["estimate", pub_cols] -->

<!-- # Map from |z| to publication weight -->
<!-- # Cutoffs in |z| space: 1.645, 1.96, 2.576 -->
<!-- # Bins: [0, 1.645], (1.645, 1.96], (1.96, 2.576], (2.576, Inf) -->
<!-- z_cutoffs  <- c(1.645, 1.96, 2.576) -->
<!-- bin_weights <- c(pub_weights, 1) # append 1 for the reference bin -->

<!-- get_pub_weight <- function(z) { -->
<!--   bin <- findInterval(abs(z), c(0, z_cutoffs, Inf)) -->
<!--   bin_weights[bin] -->
<!-- } -->


<!-- # --- 3. Per-study median-unbiased estimator (Proposition 1) ------------------ -->

<!-- # For each study with observed z, find omega such that -->
<!-- # F_{Z|Omega}(z | omega) = 0.5 -->
<!-- # where F is the CDF of the *truncated* (post-selection) distribution. -->

<!-- # CDF of the truncated z-distribution given omega and p(·) -->
<!-- F_truncated <- function(z, omega, z_cuts, weights) { -->
<!--   # Numerator: integral from -Inf to z of p(t) * phi(t - omega) dt -->
<!--   # We break this into bins and sum -->

<!--   # All bin boundaries (symmetric around 0, plus -Inf and Inf) -->
<!--   all_bounds <- sort(c(-Inf, -rev(z_cuts), 0, z_cuts, Inf)) -->
<!--   n_bins     <- length(all_bounds) - 1 -->

<!--   # Weight for each bin -->
<!--   get_w <- function(z_val) { -->
<!--     bin <- findInterval(abs(z_val), c(0, z_cuts, Inf)) -->
<!--     weights[bin] -->
<!--   } -->
...
<!--     lo <- all_bounds[k] -->
<!--     hi <- all_bounds[k + 1] -->
<!--     mid <- if (is.finite(lo) & is.finite(hi)) (lo + hi) / 2 else -->
<!--            if (is.finite(lo)) lo + 1 else hi - 1 -->
<!--     w_k <- get_w(mid) -->

<!--     # Integral of phi(t - omega) from lo to hi -->
<!--     prob_k <- pnorm(hi, mean = omega, sd = 1) - pnorm(lo, mean = omega, sd = 1) -->
<!--     denominator <- denominator + w_k * prob_k -->

<!--     # For numerator, cap the upper bound at z -->
<!--     hi_num <- min(hi, z) -->
<!--     if (hi_num > lo) { -->
<!--       prob_num <- pnorm(hi_num, mean = omega, sd = 1) - pnorm(lo, mean = omega, sd = 1) -->
<!--       numerator <- numerator + w_k * prob_num -->
<!--     } -->
<!--   } -->
...
<!-- # Find omega such that F_truncated(z, omega) = 0.5 -->
<!-- median_unbiased <- function(z, z_cuts, weights) { -->
<!--   # Root-finding: F(z | omega) - 0.5 = 0 -->
<!--   # F is decreasing in omega (larger true effect -> z is less extreme relative to it) -->
<!--   obj <- function(omega) F_truncated(z, omega, z_cuts, weights) - 0.5 -->
<!--   tryCatch( -->
<!--     uniroot(obj, interval = c(-20, 20), tol = 1e-6)$root, -->
<!--     error = \(e) NA_real_ -->
<!--   ) -->
<!-- } -->


<!-- # --- 4. Apply to each study -------------------------------------------------- -->

<!-- # Work in z-space, then convert back -->
<!-- df <- df |> -->
<!--   mutate( -->
<!--     omega_corrected = map_dbl(z_stat, \(z) { -->
<!--       median_unbiased(z, z_cutoffs, bin_weights) -->
<!--     }), -->
<!--     # Convert back to original units: theta = omega * sigma -->
<!--     estimate_corrected = omega_corrected * standard_error -->
<!--   ) -->

<!-- # Compare original vs. corrected -->
<!-- df |> -->
<!--   filter(!is.na(omega_corrected)) |> -->
<!--   summarise( -->
<!--     mean_original  = mean(estimate), -->
<!--     mean_corrected = mean(estimate_corrected), -->
<!--     median_original  = median(abs(z_stat)), -->
<!--     median_corrected = median(abs(omega_corrected)), -->
<!--     media_correction_ratio = median(abs(z_stat)/abs(omega_corrected)) -->
<!--   )  -->

<!-- # Plot: original vs. corrected z-statistics -->
<!-- df |> -->
<!--   filter(!is.na(omega_corrected)) |> -->
<!--   ggplot(aes(z_stat, omega_corrected)) + -->
<!--   geom_point(alpha = 0.5, color = "#2196b3") + -->
<!--   geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray40") + -->
<!--   labs(x = "Original z-statistic", y = "Corrected ω (median-unbiased)")  -->


<!-- # --- 5. Publication probability CI plot -------------------------------------- -->

<!-- pub_probs <- tibble( -->
<!--   bin = pub_cols, -->
<!--   est = ms$est_tab["estimate", pub_cols], -->
<!--   se  = ms$est_tab["standard error", pub_cols] -->
<!-- ) |> -->
<!--   mutate( -->
<!--     ci_lo = est - 1.96 * se, -->
<!--     ci_hi = est + 1.96 * se, -->
...
<!-- ggplot(pub_probs, aes(bin, est)) + -->
<!--   geom_point(size = 3, color = "#2196b3") + -->
<!--   geom_linerange(aes(ymin = ci_lo, ymax = ci_hi), linewidth = 0.8, color = "#2196b3") + -->
<!--   geom_hline(yintercept = 1, linetype = "dashed", color = "gray40") + -->
<!--   labs( -->
<!--     x = "|z|-statistic bin (relative to |z| > 2.576 = 1)", -->
<!--     y = "Relative publication probability", -->
<!--     caption = "Dashed line = no selection." -->
<!--   ) + -->
