...
graph_distrib_t_econ <- data_lit_causal_caliper %>%
  ...
  ggplot(., aes(x = abs_z, y = ..density.., weight = weight)) + 
  geom_histogram(bins = 50, colour = "white", alpha = 0.6) +
  geom_density(bw = 0.4, fill = NA, size = 0.9) +
...
run_logit_caliper <- function(df, fml_name, sign_level = 0.05, window_width = 0.5) {
  ...
  df_w <- df %>%
   ...
   fml_str <- switch(fml_name,
    base  = "above ~ 1",
    covar = "above ~ is_economics + is_mortality + recent + is_published + is_iv + is_reduced + is_did"
  )
...
ggplot(pub_probs, aes(bin, est)) +
  ...
  geom_errorbar(aes(ymin = ci_lo, ymax = ci_hi), width = 0.2, color = "#2196b3") +
