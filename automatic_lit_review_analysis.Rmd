...
data_retrodesign_fraction <- estimates_stats %>%
  ...
  mutate(id = 1:nrow(.)) %>%
  crossing(percentage = seq(from = 50, to = 100, by = 1) / 100) %>%
...
We display below power and exaggeration curves:
...
We display below summary statistics when the true effect is half of the estimates:
...
articles_low_adequate %>% 
  ggplot() +
  geom_density(aes(x = n_obs, color = low_power_phrase), fill = NA) +
  labs(
    title = "Distribution of the number of observations",
    # subtitle = "Comparison between articles for which an effect is retrieved and not",
    x = "Estimated number of observations",
...
articles_low_adequate %>% 
  ggplot() +
  geom_density(aes(x = length_study, color = low_power_phrase), fill = NA) +
  labs(
    title = "Distribution of the length of the study",
    # subtitle = "Comparison between articles for which an effect is retrieved and not",
    x = "Estimated length of the study (in days)",
...
articles_low_adequate %>% 
  ggplot() +
  geom_density(aes(x = n_cities, color = low_power_phrase), fill = NA) +
  labs(
