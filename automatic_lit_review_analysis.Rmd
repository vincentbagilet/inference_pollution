...
description: "In this document, we analyze abstracts to evaluate whether the literature of short terms health effects of air pollution suffers from power issues."
author:
...
Out of the `r nrow(abstracts_and_metadata)` articles returned by the query, only a fraction reports estimates and confidence intervals in their abstracts. We therefore do not retrieve effects for all the articles returned by the query, mostly for the reason aforementioned. This might create some selection, making the sample of articles studied not representative of the whole literature. We investigate further the difference between articles that do and do not report confidence intervals in their abstracts in the following section. 

We detect `r nrow(estimates)` effects and associated confidence intervals. We retrieve estimates for most of the articles mentioning "confidence interval" in some form in the abstract:  
...
Here is a list of the valid articles. 
...
<!-- effect_valid %>% mutate(meta = str_detect(abstract, "meta(\\s|-)?anal")) %>% count(meta, has_effect) -->
...
About `r articles_low_adequate %>% count(low_power) %>% mutate(prop = n/sum(n)) %>% filter(low_power) %>% .$prop*100 %>% signif(., 2)`% of estimates would not reach the conventional 80% statistical power level threshold if the true effect is 3/4 the size of the measured effect. If for all studies, the true effect was 3/4 of the measured effect, the mean type M would be about `r articles_low_adequate %>% summarise(mean_typeM = mean(typeM)) %>%  .$mean_typeM %>% signif(., 2)`. Again, this average hides a lot of heterogeneity. For the 40% of estimates with low power mentioned above, the average type M is `r articles_low_adequate %>% group_by(low_power) %>% summarise(mean_typeM = mean(typeM)) %>% filter(low_power) %>% .$mean_typeM %>% signif(., 2)`, the median is `r articles_low_adequate %>% group_by(low_power) %>% summarise(median_typeM = median(typeM)) %>% filter(low_power) %>% .$median_typeM %>% signif(., 2)`. 
