...
sim_param_evol_usual_RDD <- 
  prepare_sim_param(sim_param_unique_usual_RDD, n_iter = 3000) %>% 
...
vect_n_days <- 2200
vect_p_obs_treat <- c(0.005, 0.01, 0.02, 0.05, 0.1)
vect_percent_effect_size <- c(8, 17, 34)
vect_iv_strength <- NA
vect_formula <- c(
  "copd_age_65_75 ~ treated + temperature + temperature_squared | city + month^year + weekday"
)
...
sim_param_evol_usual_reduced <- 
  prepare_sim_param(sim_param_unique_usual_reduced, n_iter = 3000) %>% 
...
vect_iv_strength <- seq(0.1, 0.8, by = 0.1)
vect_formula <- c(
  "death_total ~ treated + temperature + temperature_squared | city + month^year + weekday")
...
sim_param_evol_usual_IV <- 
  prepare_sim_param(sim_param_unique_usual_IV, n_iter = 3000) %>% 
  filter(id_method == "IV")
```
