...
%        		Using this method, we retrieve 2,666 estimates from 784 abstracts. We then read each abstract and exclude articles whose topic falls outside the scope of our review, yielding a final corpus of 668 articles and 2,155 estimates. The sample is limited to articles that report confidence intervals and point estimates in their abstracts. We also use regular expressions to extract additional information from the abstracts, such as the pollutant and health outcome studied, the study duration, and the number of cities considered. We implement a retrospective power analysis for the 1,982 statistically significant estimates in this corpus. 
		...
		To complement this analysis, we use meta-analysis estimates from \cite{shah_short_2015} as more informed hypothetical true effect sizes for a subset of 94 studies on the effects of several air pollutants on mortality and emergency admission for stroke. For each of these studies, we run retrospective power calculations to evaluate their ability to retrieve the meta-analysis estimates. We find that 63\% have power below 80\% and a median exaggeration ratio of 1.6. As shown in \Cref{fig:retro_meta_epi}, results vary considerably by pollutant, from a median exaggeration of 1 for PM$_{2.5}$ to 13.4 for O$_3$ (the median is not displayed for visual clarity).
        ...
        Unlike the causal inference corpus, we do not observe the full set of published estimates but only those reported in abstracts. These are headline results, \textit{i.e.} the findings that authors choose to emphasize and that may be most likely to inform policymaking. This implies a specific form of selection: the selection of which results to put forward. As shown in \Cref{fig:distrib_t_stat_epi}, there is striking bunching at the 1.96 threshold, indicating that statistical significance strongly influences which results are highlighted. Since identification approaches are relatively uniform in this literature, studies are less likely to be evaluated based on their methodology, making the significance and magnitude of results all the more central to publication decisions.
